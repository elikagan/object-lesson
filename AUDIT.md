...
- [x] **P0-7 · Discount code apply at checkout — automation-verified up to Square's hosted page.** §3.3. Mechanical Playwright suite in `tests/discount-code-checkout.spec.ts` covers: (a) detail page renders the discount field on a buyable item, (b) a valid code shows strikethrough on the original price and the green discounted price next to it, (c) an invalid code surfaces the red error state and leaves the price untouched, (d) the Buy Now click POSTs `/api/checkout` with the discount code in the body. The two remaining steps that Playwright can't drive — Square's hosted checkout page rendering the discount line, and the webhook firing `used_count++` after the customer pays — collapse into a single live $1 test purchase whenever Eli is next at the register.
